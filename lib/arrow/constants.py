...
except (OverflowError, ValueError, OSError, AttributeError):  # pragma: no cover
...
try:
    ...
    # Fallback for bug in python
    is_64bits = sys.maxsize > 2 ** 32
    _MAX_ORDINAL = (
        datetime(3000, 1, 1, 23, 59, 59, 999999).toordinal()
        if is_64bits
        else datetime(2038, 1, 1, 23, 59, 59, 999999).toordinal()
    )    
...
MAX_ORDINAL: Final[int] = _MAX_ORDINAL
